# ===================================================================
# GEOPORTAL WEB - VERSIÓN 6.1 (SIDEBAR Y NAVEGACIÓN POR ÍCONOS)
# Novedades: 
# 1. Barra de Navegación Lateral (Sidebar) con logos institucionales.
# 2. Íconos curvos estilo "App" con sombras dinámicas (CSS inyectado).
# 3. Navegación directa al hacer clic en las imágenes (sin botones extra).
# ===================================================================
...
# ===================================================================
# LÓGICA DE NAVEGACIÓN POR URL (PARA HACER CLICABLES LAS IMÁGENES)
# ===================================================================
# Cuando el usuario hace clic en una imagen, la URL cambia sutilmente (Ej. ?nav=Cerrada)
if "nav" in st.query_params:
    ...
    if nav_target in ["Inicio", "Menu_Poligonales", "Menu_Altimetria", "Cerrada", "Abierta", "Niv_Cerrada", "Niv_Abierta"]:
        ...
        # Reseteamos los cálculos al cambiar de módulo
        st.session_state.calculado = False
        st.session_state.calc_niv = False
    # Limpiamos la URL para que no quede pegado el comando
    st.query_params.clear()

# ===================================================================
# FUNCIÓN MAESTRA DE RENDERIZADO (CURVAS, SOMBRAS Y CLIC)
# ===================================================================
def mostrar_icono(nombre_archivo, fallback_emoji, width=120, hover_effect=True, shadow=True, border_radius="30px", link_nav=None):
    ...
    # Búsqueda inteligente de extensión (png <-> svg)
    ...
    .{css_class} {{
        ...
        cursor: {"pointer" if link_nav else "default"};
    }}
    {hover_css}
    </style>
    """
...
# ===================================================================
with st.sidebar:
    # Logo Geopol clicable hacia el Inicio, sin sombra y sin efecto de zoom
    mostrar_icono("Logo_SinFondo.png", "🌐 Geoportal", width=220, hover_effect=False, shadow=False, border_radius="0px", link_nav="Inicio")
    ...
    st.markdown("---")
    # Logo de la Universidad estático y plano
...
PROYECCIONES_COLOMBIA = {
    "MAGNA Ciudad Bogotá (Local - ESRI:102233)": "ESRI:102233",
    "Origen Nacional CTM12 (IGAC - EPSG:9377)": "EPSG:9377",
    "MAGNA-SIRGAS Origen Central / Bogotá (EPSG:3116)": "EPSG:3116",
    "MAGNA-SIRGAS Origen Oeste / Medellín-Antioquia (EPSG:3115)": "EPSG:3115",
    "MAGNA-SIRGAS Origen Este (EPSG:3117)": "EPSG:3117",
    "UTM Zona 18N - WGS84 (EPSG:32618)": "EPSG:32618",
    "UTM Zona 19N - WGS84 (EPSG:32619)": "EPSG:32619"
}

# Resto de Variables de Poligonales
...
if st.session_state.modo_app == "Inicio":
    ...
    with col_disc1:
        # Se elimina el botón feo, ahora la imagen te lleva directamente a 'Menu_Poligonales'
...
# ===================================================================
    ...
    if st.button("🚀 Calcular Nivelación", type="primary"):
        try:
            ...
            st.error(f"❌ Error en el cálculo. Verifica los datos ingresados. Detalles: {e}")
    ...
    if st.session_state.calc_niv:
        ...
        df_rep_niv = pd.DataFrame({
            "Parámetro Evaluado": [
                "Sumatoria Vista Atrás (ΣV+)", 
                "Sumatoria Vista Adelante (ΣV-)", 
                "Cota Final Cruda (Sin Ajuste)", 
                "Cota Teórica Esperada", 
                "Error de Cierre Altimétrico (m)",
                "Error de Cierre Altimétrico (mm)"
            ],
            "Valor Obtenido": [
                f"{met['sum_vista_atras']:.3f} m", 
                f"{met['sum_vista_adelante']:.3f} m", 
                f"{met['cota_final_cruda']:.3f} m", 
                f"{met['cota_teorica_final']:.3f} m", 
                f"{met['error_cierre_m']:.4f} m",
                f"{met['error_cierre_mm']:.1f} mm"
            ]
        })
        st.table(df_rep_niv)
        ...
        perfil_chart = alt.Chart(df_plot).mark_line(
...
# ===================================================================
    ...
    with col_back:
        if st.button("⬅️ Volver a Poligonales"):
            ...
            st.rerun()
    with col_crs:
        nombre_proyeccion = st.selectbox("📍 Sistema de Referencia de Coordenadas (CRS):", list(PROYECCIONES_COLOMBIA.keys()))
        
    CRS_LOCAL = PROYECCIONES_COLOMBIA[nombre_proyeccion]
    CRS_WGS84 = "EPSG:4326"
    trans_to_local = Transformer.from_crs(CRS_WGS84, CRS_LOCAL, always_xy=True)
    trans_to_wgs = Transformer.from_crs(CRS_LOCAL, CRS_WGS84, always_xy=True)

    st.subheader(f"🛰️ Centro de Captura GPS -> Proyectando a: {CRS_LOCAL}")
    ...
    with col_gps2:
        if location and location['latitude'] is not None:
            ...
            x_plana, y_plana = trans_to_local.transform(lon_gps, lat_gps)
            st.success(f"Satélite Vinculado: Lat {lat_gps:.5f}°, Lon {lon_gps:.5f}°")
            ...
            destino = st.selectbox("¿A qué punto desea asignar esta coordenada?", opciones_destino)
            if st.button("📥 Aplicar Coordenada a Casilla", type="primary"):
                # Inyectamos el dato directamente al 'key' del widget para que la pantalla se actualice al instante
    ...
    if st.session_state.calculado:
        ...
        st.subheader(f"🗺️ Visualización Espacial Oficial ({CRS_LOCAL} a WGS84)")
