# ===================================================================
# GEOPORTAL WEB - VERSIÓN 10.0 (MEMORIA INTACTA Y GUARDADO MANUAL)
# Novedades: 
# 1. Eliminación de enlaces HTML (<a>) para evitar borrado de RAM (Hard Refresh).
# 2. Navegación 100% nativa con st.button() -> Conserva el proyecto activo.
# 3. Guardado estrictamente MANUAL (Autoguardado eliminado).
# 4. Botones de Volver y Guardar inyectados globalmente en la Barra Lateral.
# 5. Banners visibles SÓLO dentro de los módulos de trabajo.
# ===================================================================
...
def guardar_proyecto_actual(mostrar_mensaje=True):
    ...
    if st.session_state.get("proyecto_actual"):
        ...
        try:
            ...
            if mostrar_mensaje:
                st.toast(f"✅ Progreso guardado: {st.session_state.get('proyecto_actual')}", icon="💾")
                st.sidebar.success(f"💾 Guardado exitosamente a las {datetime.now().strftime('%H:%M:%S')}")
...
# ===================================================================
if st.session_state.modo_app in ["Inicio", "Menu_Principal", "Menu_Poligonales", "Menu_Altimetria"]:
...
# ===================================================================
    ...
    if calc_done:
        ...
        with st.expander("📥 Exportar Datos de Levantamiento (CAD / GIS / Reportes)", expanded=True):
            ...
            with col_tex: st.download_button(label="📄 Reporte LaTeX (.TEX)", data=data_tex, file_name=f"Reporte_{st.session_state.get('proyecto_actual') or 'Poli'}.tex", mime="text/plain", use_container_width=True)
