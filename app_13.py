# ===================================================================
# GEOPORTAL WEB - VERSIÓN 13.0 (SISTEMA DE ARCHIVOS LOCALES .GP Y MAPAS GIS)
# Novedades: 
# 1. Creación de la extensión propietaria de guardado (.gp).
# 2. Eliminación de guardado en servidor; ahora el usuario descarga sus backups.
# 3. Integración de selector de mapas base (ESRI, Google, OSM, Dark) en Nubes de Puntos.
# ===================================================================
    ...
    if st.session_state.get("proyecto_actual"):
        with st.container():
            ...
            <div style='background-color: #E3F2FD; padding: 15px; border-radius: 10px; border-left: 8px solid #2196F3; margin-bottom: 15px;'>
                ...
                <p style='margin: 0; color: #1565C0; font-size: 14px;'>Asegúrate de descargar tu archivo de seguridad para no perder tu progreso.</p>
            </div>
            """, unsafe_allow_html=True)
            
            datos_gp = generar_datos_guardado()
            st.download_button(
                label="💾 Descargar Guardado de Seguridad (.gp)",
                data=datos_gp,
        ...
        with st.expander("📥 Exportar Memorias Matemáticas y Planos (PDF / LaTeX)", expanded=True):
            ...
            if st.button("🔨 Construir y Compilar Documento Oficial", type="primary", use_container_width=True, key="btn_vol"):
                with st.spinner("Construyendo gráficas y ejecutando motor LaTeX..."):
                    ...
                    pdf_bytes, tex_vol, debug_msg = cachear_pdf_volumenes(df_calculado, df_vol_final, met, p_act, imprimir_secciones)
        ...
        with st.expander("📥 Exportar Memorias Matemáticas y Planos (PDF / LaTeX)", expanded=True):
            ...
            if st.button("🔨 Construir y Compilar Documento Oficial", type="primary", use_container_width=True, key="btn_niv"):
                with st.spinner("Construyendo gráficas y ejecutando motor LaTeX..."):
                    ...
                    pdf_bytes, tex_niv, debug_msg = cachear_pdf_altimetria(df_calc, met, p_act, tipo_niv, fotos_tomadas)
