# ===================================================================
# GEOPORTAL WEB - VERSIÓN 12.0 (MÓDULO DE NUBES DE PUNTOS GIS)
# Novedades: 
# 1. Nuevo módulo de Carga y Visualización de Nubes de Puntos (.txt / .csv).
# 2. Emparejamiento dinámico de columnas (PNEZD).
# 3. Integración de MarkerCluster en Folium para alto rendimiento espacial.
# ===================================================================
...
def obtener_b64_imagen(ruta):
    """Guarda en la memoria RAM la codificación de las imágenes."""
    with open(ruta, "rb") as f:
...
# ===================================================================
# NUEVO MÓDULO DE NUBE DE PUNTOS
# ===================================================================
elif st.session_state.modo_app == "Nube_Puntos":
    renderizar_banner_proyecto()
    st.title("📍 Visor GIS de Nubes de Puntos Topográficos")
    st.markdown("Sube tu archivo de coordenadas de campo (radiación) para previsualizar el levantamiento masivo sobre el mapa base oficial y validar la correcta georreferenciación antes de exportar a CAD.")
    ...
    st.markdown("---")
    st.subheader("1. Carga de Archivo Base")
    archivo_nube = st.file_uploader("📂 Carga tu archivo delimitado (.csv, .txt)", type=['csv', 'txt'], help="El sistema autodetectará el formato (comas, tabuladores, etc.).")
    
    if archivo_nube is not None:
        try:
            df_bruto = procesar_archivo_nube(archivo_nube)
            st.success("✅ Archivo leído con éxito.")
            with st.expander("Previsualizar Datos Crudos", expanded=False):
                st.dataframe(df_bruto.head(10), use_container_width=True)
            
            st.subheader("2. Emparejamiento de Columnas (Mapping)")
            st.info("💡 Ayuda al sistema a identificar cuáles columnas contienen las coordenadas.")
            cols = ["Ninguna"] + list(df_bruto.columns)
            c1, c2, c3, c4, c5 = st.columns(5)
            
            # Intento de adivinar columnas basado en orden clásico (PNEZD)
            col_pto = c1.selectbox("Punto / ID", cols, index=1 if len(cols)>1 else 0)
            col_e = c2.selectbox("Este (X)", cols, index=2 if len(cols)>2 else 0)
            col_n = c3.selectbox("Norte (Y)", cols, index=3 if len(cols)>3 else 0)
            col_z = c4.selectbox("Cota (Z)", cols, index=4 if len(cols)>4 else 0)
            col_desc = c5.selectbox("Descripción", cols, index=5 if len(cols)>5 else 0)
            
            if st.button("🚀 Renderizar Nube de Puntos", type="primary", use_container_width=True):
                if col_e == "Ninguna" or col_n == "Ninguna":
                    st.warning("⚠️ Debes seleccionar obligatoriamente las columnas correspondientes al Este (X) y Norte (Y).")
                else:
                    with st.spinner("Transformando coordenadas y agrupando clústers espaciales..."):
                        df_limpio = asignar_columnas(
                            df_bruto, 
                            None if col_pto == "Ninguna" else col_pto,
                            col_e,
                            col_n,
                            None if col_z == "Ninguna" else col_z,
                            None if col_desc == "Ninguna" else col_desc
                        )
                        ...
                        df_limpio['Latitud'] = latitudes
                        df_limpio['Longitud'] = longitudes
                        
                        st.markdown("---")
                        st.subheader("🗺️ Mapa Base de Levantamiento Masivo")
                        
                        centro_lat = sum(latitudes) / len(latitudes)
                        centro_lon = sum(longitudes) / len(longitudes)
                        
                        mapa_nube = folium.Map(
                        ...
                        marker_cluster = MarkerCluster(name="Nube de Puntos").add_to(mapa_nube)
                        
                        for idx, row in df_limpio.iterrows():
                            html_popup = f"<b>Punto:</b> {row['Punto']}<br><b>E:</b> {row['Este']:.3f}<br><b>N:</b> {row['Norte']:.3f}<br><b>Z:</b> {row['Cota']:.3f}<br><b>Desc:</b> {row['Descripcion']}"
                            folium.CircleMarker(
...
# ===================================================================
    ...
    if st.session_state.calc_vol:
        ...
        def cachear_pdf_volumenes(df_calc_json, df_vol_json, met, p_actual, imprimir_secciones):
            # Solución local: Des-serializamos el JSON de Pandas para independizar los datos en caché
