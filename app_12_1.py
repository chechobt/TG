# ===================================================================
# GEOPORTAL WEB - VERSIÓN 12.3 (MAPEOS INDEPENDIENTES Y FIX KEYERROR)
# Novedades: 
# 1. Emparejamiento de columnas (Mapping) independiente para cada archivo.
# 2. Solución del KeyError al subir múltiples nubes de puntos con distintos encabezados.
# 3. Vista previa de datos individualizada dentro de cada panel de archivo.
# ===================================================================
...
# ===================================================================
# GENERADORES DE CACHÉ GLOBAL (PDFS) - FUERA DE BLOQUES IF
# ===================================================================
@st.cache_data(show_spinner=False)
def cachear_pdf_volumenes(df_calculado_interno, df_vol_interno, met, p_actual, imprimir_secciones):
...
def inicializar_variables_proyecto():
    defaults = {
        ...
        "calc_vol": False,
        "nubes_guardadas": {}, # Memoria de Nubes de Puntos
...
# ===================================================================
    ...
    if st.session_state.nubes_guardadas:
        ...
        modo_vista = st.radio("Selecciona la tecnología de visualización para el mapa:", 
                            ["Agrupado (Clúster de Alto Rendimiento - Ideal para miles de puntos)", 
                             "Puntos Exactos Individuales (Precisión Detallada - Muestra el trazo topográfico real)"], 
                            horizontal=True)
        ...
        if st.button("🚀 Renderizar Múltiples Nubes", type="primary", use_container_width=True):
            ...
            if archivos_invalidos:
                ...
                with st.spinner("Transformando coordenadas y construyendo el sistema de capas espaciales (LayerControl)..."):
                    ...
                    mapa_nube = folium.Map(zoom_start=18, max_zoom=22, tiles="https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}", attr="Google Satellite")
        ...
        with st.expander("📥 Exportar Memorias Matemáticas y Planos (PDF / LaTeX)", expanded=True):
            ...
            if st.button("🔨 Construir y Compilar Documento Oficial", type="primary", use_container_width=True, key="btn_niv"):
                with st.spinner("Construyendo gráficas y ejecutando motor LaTeX..."):
                    ...
                    tex_niv = generar_reporte_nivelacion_latex(df_calc, met, tipo_niv, autores, tutor, path_grafico=ruta_perfil, fotos_paths=fotos_tomadas)
        ...
        with st.expander("📥 Exportar Datos de Levantamiento (CAD / GIS / Reportes)", expanded=True):
            ...
            if st.button("🔨 Construir y Compilar Documento Oficial", type="primary", use_container_width=True, key="btn_poli"):
                with st.spinner("Compilando Documento LaTeX..."):
                    ...
                    pdf_bytes, data_tex, debug_msg = cachear_pdf_poli(df_campo, df_ajuste, met, p_act, ruta_plano_export, fotos_tomadas, st.session_state.modo_app)
