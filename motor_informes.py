# ===================================================================
# MOTOR DE GENERACIÓN DE INFORMES TÉCNICOS EN LATEX
# Desarrollado para Geoportal Web (GeoPol)
#
# v2 — mejoras de contenido técnico:
#   - Resumen ejecutivo al inicio de cada informe
#   - Ficha técnica de campo opcional (equipo, clima, cuadrilla, fecha)
#   - Dictamen + recomendaciones estructuradas (antes: un solo párrafo)
#   - Sección de referencias y marco normativo por tipo de informe
#   - Nivelación: dictamen de tolerancia (criterio T = ek·√K) y tabla
#     opcional de Bancos de Nivel utilizados
#   - Volúmenes: sección opcional de análisis de acarreo/compensación
#   - Todos los parámetros nuevos son opcionales -> no rompe llamadas
#     existentes desde la web
# ===================================================================
import pandas as pd
import numpy as np
import re
import math
from datetime import datetime
import os
import subprocess
import shutil
import platform

def escapar_latex(texto):
    if not isinstance(texto, str):
        texto = str(texto)
    reemplazos = {
        '&': r'\&', '%': r'\%', '$': r'\$', '#': r'\#', '_': r'\_',
        '{': r'\{', '}': r'\}', '~': r'\textasciitilde{}', '^': r'\textasciicircum{}',
    }
    tx = texto
    for key, val in reemplazos.items():
        tx = tx.replace(key, val)
    return tx


# -------------------------------------------------------------------
# TABLAS
# -------------------------------------------------------------------

def dividir_y_generar_tablas(df, caption, label_prefix, id_cols=2):
    """
    Divide un DataFrame ancho en varias tablas verticales de máximo 6 columnas
    para mantener el formato vertical (portrait) perfectamente legible.
    Mantiene las primeras 'id_cols' columnas como referencia.
    """
    cols = list(df.columns)
    if len(cols) <= (id_cols + 4):
        return dataframe_a_latex_table(df, caption, label_prefix)

    base_cols = cols[:id_cols]
    rest_cols = cols[id_cols:]

    chunks = [rest_cols[i:i+4] for i in range(0, len(rest_cols), 4)]

    latex_str = ""
    for i, chunk in enumerate(chunks):
        sub_df = df[base_cols + chunk]
        latex_str += dataframe_a_latex_table(sub_df, f"{caption} (Parte {i+1})", f"{label_prefix}_{i}")
        latex_str += "\n\\vspace{0.8cm}\n"
    return latex_str


def dataframe_a_latex_table(df, caption, label):
    num_columnas = len(df.columns)
    latex_tab = []
    latex_tab.append(r"\begin{table}[H]")
    latex_tab.append(r"  \centering")
    latex_tab.append(r"  \small")
    latex_tab.append(f"  \\caption{{{escapar_latex(caption)}}}")
    latex_tab.append(f"  \\label{{tab:{label}}}")
    alineacion = "|" + "|".join(["c"] * num_columnas) + "|"
    latex_tab.append(f"  \\begin{{tabular}}{{{alineacion}}}")
    latex_tab.append(r"    \hline")
    latex_tab.append(r"    \rowcolor{GeoBlue}")

    headers = [f"\\textcolor{{white}}{{\\textbf{{{escapar_latex(col)}}}}}" for col in df.columns]
    latex_tab.append("    " + " & ".join(headers) + r" \\")
    latex_tab.append(r"    \hline")

    for idx, row in df.iterrows():
        fila_items = []
        for col in df.columns:
            val = row[col]
            if pd.isna(val) or val is None:
                fila_items.append("---")
            elif isinstance(val, (int, float, np.number)):
                # Nota: se estandariza a 3 decimales independientemente de la
                # magnitud (la distinción previa por encima/debajo de 10000
                # no producía ninguna diferencia real de formato).
                fila_items.append(f"{val:.3f}")
            else:
                fila_items.append(escapar_latex(str(val)))

        if idx % 2 == 0:
            latex_tab.append(r"    \rowcolor{GeoBlue!5}")
        else:
            latex_tab.append(r"    \rowcolor{white}")

        latex_tab.append("    " + " & ".join(fila_items) + r" \\")
        latex_tab.append(r"    \hline")

    latex_tab.append(r"  \end{tabular}")
    latex_tab.append(r"\end{table}")

    return "\n".join(latex_tab)


def generar_tabla_resumen_ejecutivo(pares, titulo="Resumen Ejecutivo de Resultados"):
    """
    pares: lista de tuplas (etiqueta, valor) ya formateadas como texto.
    Genera una tabla compacta de dos columnas para insertar al inicio
    del informe, antes de entrar en el detalle metodológico.
    """
    tex = []
    tex.append(r"\section{" + titulo + r"}")
    tex.append(r"La siguiente tabla resume los resultados más relevantes del procesamiento, "
                r"a manera de consulta rápida antes de revisar el detalle metodológico y las carteras completas.")
    tex.append(r"\begin{table}[H]")
    tex.append(r"  \centering")
    tex.append(r"  \begin{tabular}{|l|r|}")
    tex.append(r"    \hline")
    tex.append(r"    \rowcolor{GeoBlue}")
    tex.append(r"    \textcolor{white}{\textbf{Parámetro}} & \textcolor{white}{\textbf{Valor}} \\")
    tex.append(r"    \hline")
    for i, (etiqueta, valor) in enumerate(pares):
        tex.append(r"    \rowcolor{GeoBlue!5}" if i % 2 == 0 else r"    \rowcolor{white}")
        tex.append(f"    {escapar_latex(etiqueta)} & {valor} \\\\")
        tex.append(r"    \hline")
    tex.append(r"  \end{tabular}")
    tex.append(r"\end{table}")
    return "\n".join(tex)


def generar_ficha_tecnica_campo(metadata):
    """
    metadata: dict opcional con llaves como:
        'fecha', 'equipo', 'operador', 'cuadrilla', 'clima', 'metodo'
    Cualquier llave ausente simplemente no se imprime. Si metadata es
    None o está vacío, no se genera ninguna sección.
    """
    if not metadata:
        return ""

    etiquetas = {
        'fecha': 'Fecha de Observación',
        'equipo': 'Equipo / Instrumento Utilizado',
        'operador': 'Operador Responsable',
        'cuadrilla': 'Cuadrilla de Campo',
        'clima': 'Condiciones Climáticas',
        'metodo': 'Método de Observación',
    }

    tex = []
    tex.append(r"\subsection{Ficha Técnica del Levantamiento}")
    tex.append(r"\begin{table}[H]")
    tex.append(r"  \centering")
    tex.append(r"  \begin{tabular}{|l|l|}")
    tex.append(r"    \hline")
    filas = 0
    for key, etiqueta in etiquetas.items():
        if key in metadata and metadata[key]:
            tex.append(r"    \rowcolor{GeoBlue!5}" if filas % 2 == 0 else r"    \rowcolor{white}")
            tex.append(f"    \\textbf{{{etiqueta}}} & {escapar_latex(str(metadata[key]))} \\\\")
            tex.append(r"    \hline")
            filas += 1

    if filas == 0:
        return ""

    tex.append(r"  \end{tabular}")
    tex.append(r"\end{table}")
    return "\n".join(tex)


# -------------------------------------------------------------------
# EVALUACIONES / DICTÁMENES (dictamen + lista de recomendaciones)
# -------------------------------------------------------------------

def evaluar_precision(prec_h):
    if prec_h <= 0:
        dictamen = ("La poligonal presenta un error matemático crítico o no logró cerrar "
                     "geométricamente sobre el punto o azimut de control.")
        recomendaciones = [
            "Revisar la cartera de campo en busca de errores de digitación, transposición de cifras o de lectura angular.",
            "Verificar que el punto de amarre inicial (coordenadas y azimut de partida) corresponda efectivamente a puntos de control válidos y vigentes.",
            "Repetir la observación de campo del tramo con mayor discrepancia antes de continuar con el procesamiento.",
        ]
    elif prec_h < 1000:
        dictamen = (f"Se obtuvo una precisión de 1:{int(prec_h)}. Esta precisión se clasifica como "
                     r"\textbf{DEFICIENTE} y no cumple con los estándares mínimos exigidos para levantamientos "
                     "topográficos convencionales.")
        recomendaciones = [
            "Repetir la totalidad de las observaciones angulares y de distancia en campo.",
            "Verificar la calibración y la correcta nivelación del instrumento utilizado.",
            "Descartar errores groseros de lectura revisando estación por estación en la cartera original.",
        ]
    elif prec_h < 5000:
        dictamen = (f"Se obtuvo una precisión de 1:{int(prec_h)}. Esta precisión se clasifica como "
                     r"\textbf{BAJA}. Es aceptable únicamente para levantamientos rurales expeditos o "
                     "estimaciones preliminares, pero carece de la fiabilidad necesaria para la ejecución "
                     "de obra civil de detalle.")
        recomendaciones = [
            "Utilizar el resultado únicamente como referencia preliminar, no como insumo de diseño definitivo.",
            "De ser posible, repetir las estaciones con mayor error de cierre parcial identificado.",
            "Evaluar el uso de un instrumento de mayor precisión angular (p. ej. estación total en lugar de teodolito óptico).",
        ]
    elif prec_h < 15000:
        dictamen = (f"Se obtuvo una precisión de 1:{int(prec_h)}. Esta precisión se clasifica como "
                     r"\textbf{BUENA}. Cumple plenamente con los estándares vigentes para levantamientos "
                     "urbanos, topografía convencional y diseño de obras civiles de rigor intermedio.")
        recomendaciones = [
            "Documentar el instrumental y el procedimiento de observación empleados, para trazabilidad del proyecto.",
            "La red es apta como insumo directo de diseño para obras civiles de exigencia intermedia.",
        ]
    else:
        dictamen = (f"Se obtuvo una precisión de 1:{int(prec_h)}. Esta precisión se clasifica como "
                     r"\textbf{ALTA}. Demuestra un excelente trabajo de campo y de instrumentación.")
        recomendaciones = [
            "Este nivel de error es aplicable a redes de control de alta exigencia, infraestructura pesada o trazados geodésicos.",
            "Se recomienda conservar esta red como punto de control permanente para densificaciones futuras del proyecto.",
        ]
    return dictamen, recomendaciones


def evaluar_volumen(neto, corte, relleno, analisis_acarreo=None):
    if neto > 0:
        dictamen = ("Dado el balance volumétrico resultante, el proyecto requiere transportar material "
                     "excedentario hacia un sitio de disposición final (botadero), ya que los volúmenes de "
                     "excavación superan a los terraplenes requeridos.")
        recomendaciones = [
            "Identificar y gestionar la autorización de un sitio de disposición final de material sobrante conforme a la normativa ambiental vigente.",
            "Considerar el factor de esponjamiento del material de corte al dimensionar el número de viajes de volqueta requeridos.",
            "Planificar el acarreo priorizando los tramos con mayor excedente para optimizar distancias de transporte.",
        ]
    elif neto < 0:
        dictamen = ("El diseño geométrico evaluado requiere la importación de material de préstamo, dado que "
                     "el volumen de relleno (terraplén) supera la cantidad de material obtenido por excavación.")
        recomendaciones = [
            "Identificar y gestionar la autorización de una fuente de material de préstamo cercana al proyecto.",
            "Considerar el factor de compactación del material de préstamo al calcular el volumen a adquirir en banco.",
            "Evaluar si un ajuste del diseño geométrico (rasante) permite reducir el déficit de material.",
        ]
    else:
        dictamen = ("El diseño presenta una compensación volumétrica prácticamente perfecta, optimizando al "
                     "máximo los costos de movimiento de tierras y transporte.")
        recomendaciones = [
            "Aprovechar la compensación planificando el movimiento interno de tierras por tramos, minimizando distancias de acarreo.",
            "Verificar que la secuencia constructiva respete el diagrama de masas para no generar sobrecostos por doble manejo de material.",
        ]

    if analisis_acarreo:
        dist_libre = analisis_acarreo.get('distancia_libre_km')
        costo_m3 = analisis_acarreo.get('costo_m3')
        if dist_libre is not None:
            recomendaciones.append(
                f"Se ha considerado una distancia libre de acarreo de {dist_libre:.2f} km; los volúmenes que "
                "superen dicha distancia deben presupuestarse como transporte adicional."
            )
        if costo_m3 is not None:
            recomendaciones.append(
                f"Con base en el costo unitario suministrado ({costo_m3:.2f} por m³ transportado), se recomienda "
                "validar esta cifra con el presupuesto oficial del proyecto antes de su uso contractual."
            )

    return dictamen, recomendaciones


def evaluar_nivelacion(error_mm, distancia_km=None, coef_precision_mm=10.0):
    """
    Criterio general de topografía: la tolerancia de cierre de un circuito
    de nivelación suele expresarse como T = ek * sqrt(K), donde K es la
    longitud del circuito en kilómetros y ek ('error kilométrico admisible')
    depende del tipo de nivelación:
        - Alta precisión / geodésica:  ek ≈ 4-5 mm
        - Precisión ordinaria (obras civiles, vías): ek ≈ 8-12 mm
        - Reconocimiento / estudios preliminares: ek ≈ 20-30 mm
    coef_precision_mm permite ajustar ek según el criterio del proyecto
    o de la institución (por defecto 10 mm, valor típico de nivelación
    ordinaria). Si no se conoce la distancia recorrida, se usa un criterio
    cualitativo basado únicamente en la magnitud del error en mm.
    """
    error_abs = abs(error_mm)

    if distancia_km and distancia_km > 0:
        tolerancia = coef_precision_mm * math.sqrt(distancia_km)
        dentro_tolerancia = error_abs <= tolerancia
        dictamen = (
            f"Para un recorrido de {distancia_km:.3f} km y un coeficiente de precisión de "
            f"{coef_precision_mm:.1f} mm/$\\sqrt{{\\text{{km}}}}$, la tolerancia admisible es de "
            f"{tolerancia:.1f} mm. El error de cierre obtenido fue de {error_abs:.1f} mm, por lo tanto "
            + (r"el circuito se encuentra \textbf{DENTRO} de la tolerancia y es apto para compensación."
               if dentro_tolerancia else
               r"el circuito se encuentra \textbf{FUERA} de la tolerancia admisible.")
        )
        if dentro_tolerancia:
            recomendaciones = [
                "Proceder con la compensación proporcional del error entre los puntos de cambio.",
                "Conservar el registro del coeficiente de precisión utilizado para trazabilidad del criterio de aceptación.",
            ]
        else:
            recomendaciones = [
                "Repetir el circuito de nivelación, priorizando los tramos con mayor número de puntos de cambio.",
                "Verificar la correcta compensación del error de índice (colimación) del nivel utilizado.",
                "Evaluar si el coeficiente de precisión exigido por el proyecto es más estricto que el asumido por defecto.",
            ]
    else:
        if error_abs < 5:
            dictamen = f"El error de cierre obtenido ({error_abs:.1f} mm) es muy bajo, propio de nivelación de precisión."
            recomendaciones = ["Apto para control vertical de alta exigencia."]
        elif error_abs < 15:
            dictamen = f"El error de cierre obtenido ({error_abs:.1f} mm) es aceptable para nivelación ordinaria de obras civiles."
            recomendaciones = ["Apto para densificación de cotas y ejecución de obra civil convencional."]
        elif error_abs < 30:
            dictamen = f"El error de cierre obtenido ({error_abs:.1f} mm) es admisible solo para trabajos preliminares."
            recomendaciones = ["Usar el resultado como referencia preliminar; considerar repetir el circuito para trabajos de detalle."]
        else:
            dictamen = f"El error de cierre obtenido ({error_abs:.1f} mm) es elevado."
            recomendaciones = [
                "Repetir el circuito de nivelación revisando la correcta lectura de vistas atrás/adelante.",
                "Verificar la calibración del nivel (error de colimación) antes de una nueva salida de campo.",
            ]

    return dictamen, recomendaciones


def generar_seccion_recomendaciones(dictamen, recomendaciones, texto_cierre=None):
    tex = []
    tex.append(r"\subsection{Dictamen General}")
    tex.append(dictamen)
    tex.append(r"\subsection{Recomendaciones Técnicas}")
    tex.append(r"\begin{itemize}")
    for r in recomendaciones:
        tex.append(f"  \\item {r}")
    tex.append(r"\end{itemize}")
    if texto_cierre:
        tex.append(texto_cierre)
    return "\n".join(tex)


# -------------------------------------------------------------------
# REFERENCIAS Y MARCO NORMATIVO
# -------------------------------------------------------------------

def generar_referencias_normativas(tipo):
    """
    tipo: 'poligonal' | 'volumenes' | 'nivelacion'
    Bibliografía base común + ítems específicos por tipo de informe.
    """
    tex = []
    tex.append(r"\section{Referencias y Marco Normativo}")
    tex.append(r"\begin{itemize}")
    tex.append(r"  \item Instituto Geográfico Agustín Codazzi (IGAC). \textit{Resolución 471 de 2020}, "
               r"por la cual se adopta el marco geocéntrico nacional de referencia MAGNA-SIRGAS como único "
               r"sistema de coordenadas oficial de Colombia.")
    tex.append(r"  \item Instituto Geográfico Agustín Codazzi (IGAC). \textit{Resolución 529 de 2020}, "
               r"modificatoria de la Resolución 471 de 2020.")

    if tipo == 'poligonal':
        tex.append(r"  \item Casanova, L. \textit{Topografía Plana}. Universidad de los Andes, Facultad de Ingeniería.")
        tex.append(r"  \item McCormac, J. \textit{Topografía}. Editorial Limusa Wiley.")
        tex.append(r"  \item Criterio de compensación de poligonales por el Método de la Brújula (Regla de Bowditch).")
    elif tipo == 'volumenes':
        tex.append(r"  \item Instituto Nacional de Vías (INVIAS). \textit{Manual de Diseño Geométrico de Carreteras}.")
        tex.append(r"  \item Método de las Áreas Medias (Average End Area Method) para cálculo de volúmenes de movimiento de tierras.")
        tex.append(r"  \item Ministerio de Ambiente y Desarrollo Sostenible -- normatividad ambiental vigente aplicable a sitios de disposición final de material sobrante.")
    elif tipo == 'nivelacion':
        tex.append(r"  \item Criterio general de tolerancia de cierre en nivelación geométrica: $T = e_k \sqrt{K}$, "
                   r"donde $K$ es la longitud del circuito en kilómetros y $e_k$ el error kilométrico admisible "
                   r"según el tipo de nivelación (alta precisión, ordinaria o de reconocimiento).")
        tex.append(r"  \item Wolf, P. \& Ghilani, C. \textit{Topografía}. Pearson Educación -- capítulo de Nivelación Diferencial.")

    tex.append(r"\end{itemize}")
    tex.append(r"\textit{Nota: las referencias normativas y bibliográficas deben verificarse frente a la "
               r"versión vigente al momento de la entrega del proyecto, dado que resoluciones y manuales "
               r"pueden ser actualizados por las entidades correspondientes.}")
    return "\n".join(tex)


# -------------------------------------------------------------------
# PORTADA / PREÁMBULO
# -------------------------------------------------------------------

def generar_preambulo_y_caratula(titulo_informe, autores, tutor, tipo_poligonal=None):
    tex = []
    tex.append(r"\documentclass[11pt,letterpaper]{article}")
    tex.append(r"\usepackage[utf8]{inputenc}")
    tex.append(r"\usepackage[spanish,es-tabla]{babel}")
    tex.append(r"\usepackage[margin=2.5cm]{geometry}")
    tex.append(r"\usepackage{amsmath,amssymb}")
    tex.append(r"\usepackage{booktabs}")
    tex.append(r"\usepackage{graphicx}")
    tex.append(r"\usepackage{float}")
    tex.append(r"\usepackage{fancyhdr}")
    tex.append(r"\usepackage{hyperref}")
    tex.append(r"\usepackage[table]{xcolor}")
    tex.append(r"\usepackage{tikz}")
    tex.append(r"\usepackage{transparent}")
    tex.append(r"\usepackage{eso-pic}")
    tex.append(r"\usepackage{caption}")

    tex.append(r"\definecolor{GeoOrange}{HTML}{FF8C00}")
    tex.append(r"\definecolor{GeoBlue}{HTML}{0D47A1}")

    # MARCA DE AGUA (WATERMARK)
    ruta_logo = "Iconos/logo_geopol.png"
    if os.path.exists(ruta_logo):
        ruta_logo_latex = ruta_logo.replace('\\', '/')
        tex.append(r"\AddToShipoutPictureBG{")
        tex.append(r"  \AtPageCenter{")
        tex.append(f"    \\makebox[0pt]{{\\transparent{{0.06}}\\includegraphics[width=12cm]{{{ruta_logo_latex}}}}}")
        tex.append(r"  }")
        tex.append(r"}")

    tex.append(r"\hypersetup{colorlinks=true, linkcolor=GeoBlue, urlcolor=GeoOrange}")
    tex.append(r"\pagestyle{fancy}")
    tex.append(r"\fancyhf{}")
    tex.append(r"\fancyhead[L]{\footnotesize \textcolor{GeoBlue}{\textbf{GeoPol Web}} - Reporte Técnico}")
    tex.append(r"\fancyhead[R]{\footnotesize Universidad Distrital F.J.C.}")
    tex.append(r"\fancyfoot[C]{\thepage}")
    tex.append(r"\renewcommand{\headrulewidth}{0.4pt}")
    tex.append(r"\renewcommand{\footrulewidth}{0.4pt}")

    tex.append(r"\begin{document}")

    # PORTADA
    tex.append(r"\begin{titlepage}")
    tex.append(r"\begin{tikzpicture}[remember picture,overlay]")
    tex.append(r"  \fill[GeoBlue] (current page.north west) rectangle ([yshift=-4cm]current page.north east);")
    tex.append(r"  \fill[GeoOrange] ([yshift=-4cm]current page.north west) rectangle ([yshift=-4.5cm]current page.north east);")
    tex.append(r"  \fill[GeoBlue!5] (current page.south west) rectangle ([yshift=4cm]current page.south east);")
    tex.append(r"\end{tikzpicture}")

    tex.append(r"\vspace*{-2cm}")
    tex.append(r"\begin{center}")
    tex.append(r"  \textcolor{white}{\Huge \textbf{GEOPORTAL WEB (GeoPol)}} \\[0.5cm]")
    tex.append(r"  \textcolor{white}{\large \textit{''Máxima precisión al alcance de tus manos''}} \\[2cm]")
    tex.append(r"  \vspace{3cm}")
    tex.append(r"  {\LARGE \textbf{INFORME TÉCNICO DE INGENIERÍA}} \\[0.5cm]")
    if tipo_poligonal:
        tex.append(f"  {{\\Large \\textbf{{{tipo_poligonal}}}}} \\\\[2cm]")
    else:
        tex.append(f"  {{\\Large \\textbf{{{titulo_informe}}}}} \\\\[2cm]")

    tex.append(r"  \begin{flushleft}")
    tex.append(r"    \Large \textbf{Autores del Procesamiento:}\\[0.2cm]")
    for aut in autores:
        tex.append(f"    \\large $\\bullet$ {escapar_latex(aut)} \\\\[0.1cm]")
    tex.append(r"    \vspace{1cm}")
    tex.append(r"    \Large \textbf{Tutor - Director de Proyecto:}\\[0.2cm]")
    tex.append(f"    \\large $\\bullet$ {escapar_latex(tutor)}")
    tex.append(r"  \end{flushleft}")
    tex.append(r"  \vfill")
    tex.append(r"  \textbf{UNIVERSIDAD DISTRITAL FRANCISCO JOSÉ DE CALDAS}\\[0.2cm]")
    tex.append(r"  Facultad de Medio Ambiente y Recursos Naturales \\ Ingeniería Topográfica / Civil \\[0.2cm]")
    tex.append(f"  Bogotá D.C. -- {datetime.now().strftime('%d de %B de %Y')}")
    tex.append(r"\end{center}")
    tex.append(r"\end{titlepage}")

    tex.append(r"\tableofcontents")
    tex.append(r"\newpage")
    return "\n".join(tex)


# -------------------------------------------------------------------
# INFORME: POLIGONALES
# -------------------------------------------------------------------

def generar_reporte_poligonal_latex(df_campo, df_ajuste, metricas, tipo_poligonal, autores, tutor,
                                     path_grafico=None, fotos_paths=None,
                                     metadata_campo=None, puntos_referencia=None):
    """
    Parámetros nuevos (opcionales, no rompen llamadas existentes):
      metadata_campo: dict con 'fecha','equipo','operador','cuadrilla','clima','metodo'
      puntos_referencia: lista de strings describiendo los puntos IGAC / de control usados para el amarre
    """
    tex = [generar_preambulo_y_caratula(tipo_poligonal, autores, tutor, tipo_poligonal)]

    # --- RESUMEN EJECUTIVO ---
    prec_h = metricas.get('prec_h', 0)
    n_vertices = len(df_ajuste) if df_ajuste is not None else 0
    pares_resumen = [
        ("Tipo de poligonal", escapar_latex(tipo_poligonal)),
        ("Número de vértices procesados", str(n_vertices)),
        ("Error angular bruto", escapar_latex(str(metricas.get('err_ang_ant', 0)))),
        ("Error lineal de cierre ($e_L$)", f"{metricas.get('err_h_ant', 0):.5f} m"),
        ("Precisión planimétrica", f"1 : {int(prec_h)}" if prec_h > 0 else "No cierra"),
        ("Error vertical ($\\Delta Z$)", f"{metricas.get('err_v_ant', 0):.5f} m"),
    ]
    tex.append(generar_tabla_resumen_ejecutivo(pares_resumen))

    ficha = generar_ficha_tecnica_campo(metadata_campo)
    if ficha:
        tex.append(ficha)

    if puntos_referencia:
        tex.append(r"\subsection{Puntos de Control y Amarre Utilizados}")
        tex.append(r"\begin{itemize}")
        for p in puntos_referencia:
            tex.append(f"  \\item {escapar_latex(str(p))}")
        tex.append(r"\end{itemize}")

    tex.append(r"\section{Marco Teórico y Referencia Geodésica}")
    tex.append(r"El presente informe detalla el cálculo, ajuste y representación de una red de apoyo planimétrico. Este procesamiento se apoya en los lineamientos técnicos de la Topografía Clásica y la normatividad geodésica dictada por el \textbf{Instituto Geográfico Agustín Codazzi (IGAC)}.")
    tex.append(r"\subsection{Sistema de Georreferenciación (MAGNA-SIRGAS)}")
    tex.append(r"De acuerdo con la \textbf{Resolución 471 de 2020} (y su modificación en la Resolución 529 de 2020) emitida por el IGAC, el único sistema oficial de coordenadas para la República de Colombia es el Origen Nacional \textbf{MAGNA-SIRGAS (EPSG: 9377)}.")

    tex.append(r"\subsection{Fundamento Matemático: " + tipo_poligonal + "}")
    if "Cerrada" in tipo_poligonal:
        tex.append(r"Una Poligonal Cerrada de Circuito es aquella que inicia en una estación conocida y, tras medir una serie de vértices (deltas), retorna matemáticamente al mismo punto de origen. Esto permite una doble comprobación de errores:")
        tex.append(r"\begin{itemize}")
        tex.append(r"  \item \textbf{Cierre Angular:} La suma teórica de los ángulos internos debe cumplir $\Sigma \alpha = (n - 2) \cdot 180^\circ$. El error angular se compensa en partes iguales o ponderadas.")
        tex.append(r"  \item \textbf{Cierre Lineal:} Las sumatorias de las proyecciones corregidas en el eje Norte ($Y$) y Este ($X$) deben ser estrictamente cero ($\Sigma \Delta N = 0, \Sigma \Delta E = 0$). El error de cierre se ajusta mediante el método de la Brújula (Regla de Bowditch).")
        tex.append(r"\end{itemize}")
    else:
        tex.append(r"Una Poligonal Abierta con Control es aquella que parte de una línea base con azimut y coordenadas conocidas, y finaliza su recorrido en otra estación (o par de estaciones) de coordenadas igualmente conocidas.")
        tex.append(r"\begin{itemize}")
        tex.append(r"  \item \textbf{Cierre Angular:} El azimut calculado del último alineamiento se compara contra el azimut teórico conocido de llegada.")
        tex.append(r"  \item \textbf{Cierre Lineal:} La sumatoria de las proyecciones debe ser igual a la diferencia real de coordenadas entre el punto de llegada y el de partida ($\Sigma \Delta N = N_{llegada} - N_{partida}$). Los desvíos se compensan proporcionalmente a las distancias.")
        tex.append(r"\end{itemize}")

    tex.append(r"\section{Trabajo de Campo: Registro de Observaciones}")
    tex.append(r"En la siguiente sección se relacionan los datos brutos levantados en campo (ángulos horizontales, verticales, distancias inclinadas y alturas instrumentales).")

    df_c_clean = df_campo.drop(columns=['📸 Tomar_Fotos'], errors='ignore')
    tex.append(dividir_y_generar_tablas(df_c_clean, "Cartera de Observaciones Brutas", "campo"))

    if fotos_paths and len(fotos_paths) > 0:
        tex.append(r"\subsection{Registro Fotográfico Panorámico}")
        tex.append(r"\begin{figure}[H]")
        tex.append(r"  \centering")
        for idx, path in enumerate(fotos_paths[:4]):
            path_latex = path.replace('\\', '/')
            tex.append(f"  \\includegraphics[height=5cm, keepaspectratio]{{{path_latex}}}")
            if idx % 2 == 1:
                tex.append(r"  \\[0.5cm]")
        tex.append(r"  \caption{Mosaico de registro fotográfico de estaciones}")
        tex.append(r"\end{figure}")

    tex.append(r"\section{Cálculo, Análisis de Errores y Compensación}")
    tex.append(r"El motor de cálculo topográfico procesó las observaciones, obteniendo las siguientes métricas de cierre (incluyendo planimetría y altimetría) antes de ejecutar el ajuste perimetral:")

    tex.append(r"\begin{table}[H]")
    tex.append(r"  \centering")
    tex.append(r"  \caption{Métricas de Cierre Geométrico}")
    tex.append(r"  \begin{tabular}{|l|l|}")
    tex.append(r"    \hline")
    tex.append(r"    \rowcolor{GeoBlue}")
    tex.append(r"    \textcolor{white}{\textbf{Parámetro Analizado}} & \textcolor{white}{\textbf{Magnitud del Error}} \\")
    tex.append(r"    \hline")
    tex.append(f"    Error Angular Bruto & {escapar_latex(str(metricas.get('err_ang_ant', 0)))} \\\\")
    tex.append(r"    \rowcolor{GeoBlue!5}")
    tex.append(f"    Error Horizontal Este ($e_x$) & {metricas.get('err_e_ant', 0):.5f} m \\\\")
    tex.append(f"    Error Horizontal Norte ($e_y$) & {metricas.get('err_n_ant', 0):.5f} m \\\\")
    tex.append(r"    \rowcolor{white}")
    tex.append(f"    Error Vertical ($\\Delta Z$) & {metricas.get('err_v_ant', 0):.5f} m \\\\")
    tex.append(f"    Precisión Vertical Relativa & 1 en {int(metricas.get('prec_v', 0))} \\\\")
    tex.append(r"    \rowcolor{GeoBlue!5}")
    tex.append(f"    Error Lineal Cierre ($e_L$) & {metricas.get('err_h_ant', 0):.5f} m \\\\")
    tex.append(f"    Precisión Planimétrica ($1:P$) & 1 en {int(prec_h)} \\\\")
    tex.append(r"    \hline")
    tex.append(r"  \end{tabular}")
    tex.append(r"\end{table}")

    tex.append(r"\subsection{Cartera Final de Coordenadas Ajustadas}")
    tex.append(dividir_y_generar_tablas(df_ajuste, "Coordenadas Compensadas de la Red", "ajuste"))

    if path_grafico:
        path_grafico_latex = path_grafico.replace('\\', '/')
        tex.append(r"\section{Esquema Geométrico de la Red Planimétrica}")
        tex.append(r"\begin{figure}[H]")
        tex.append(r"  \centering")
        tex.append(f"  \\includegraphics[width=0.95\\textwidth]{{{path_grafico_latex}}}")
        tex.append(f"  \\caption{{Plano As-Built de la {tipo_poligonal}}}")
        tex.append(r"\end{figure}")

    tex.append(r"\section{Conclusiones y Dictamen Técnico}")
    dictamen, recomendaciones = evaluar_precision(prec_h)
    texto_cierre = (r"El procesamiento fue realizado exitosamente de forma automatizada mediante "
                     r"\textbf{GeoPol Web}, suprimiendo el error de cálculo humano y garantizando la "
                     r"trazabilidad matemática requerida por la ingeniería civil moderna.")
    tex.append(generar_seccion_recomendaciones(dictamen, recomendaciones, texto_cierre))

    tex.append(generar_referencias_normativas('poligonal'))

    tex.append(r"\end{document}")
    return "\n".join(tex)


# -------------------------------------------------------------------
# INFORME: VOLÚMENES
# -------------------------------------------------------------------

def generar_reporte_volumenes_latex(df_cubicaje, metricas, autores, tutor,
                                     path_grafico=None, path_masas=None, paths_secciones=None,
                                     analisis_acarreo=None):
    """
    Parámetro nuevo (opcional):
      analisis_acarreo: dict opcional con 'distancia_libre_km' y/o 'costo_m3'
    """
    titulo = "Memorias de Cálculo Matemático y Diseño Vial\\\\ (Cubicaje de Volúmenes)"
    tex = [generar_preambulo_y_caratula(titulo, autores, tutor)]

    corte = metricas.get('Corte_Total', 0)
    relleno = metricas.get('Relleno_Total', 0)
    neto = metricas.get('Volumen_Neto', 0)

    # --- RESUMEN EJECUTIVO ---
    pares_resumen = [
        ("Volumen total de corte", f"{corte:.3f} m$^3$"),
        ("Volumen total de relleno", f"{relleno:.3f} m$^3$"),
        ("Balance neto del proyecto", f"{neto:.3f} m$^3$"),
        ("Condición predominante", "Excedente de corte" if neto > 0 else ("Déficit / material de préstamo" if neto < 0 else "Compensado")),
    ]
    tex.append(generar_tabla_resumen_ejecutivo(pares_resumen))

    tex.append(r"\section{Introducción y Metodología de Cubicaje}")
    tex.append(r"Este reporte detalla el cálculo de movimiento de tierras para el proyecto vial. El cálculo de volúmenes se ejecutó mediante el \textbf{Método de las Áreas Medias (Average End Area Method)}, evaluando sección transversal por sección transversal de forma secuencial.")

    tex.append(r"\subsection{Criterios del Diagrama de Masas (Curva Masa)}")
    tex.append(r"Para el análisis del movimiento de tierras, se ha calculado el volumen neto acumulado. La Curva Masa permite visualizar analíticamente el déficit o superávit de material a lo largo del eje del proyecto, facilitando el diseño de las zonas de préstamo y botadero.")

    tex.append(r"\section{Cuadro de Movimiento de Tierras}")
    tex.append(r"A continuación, se presenta la tabla de cálculo tabulada. Los valores negativos indican predominancia de relleno, mientras que los positivos corresponden a áreas de corte.")
    tex.append(dividir_y_generar_tablas(df_cubicaje, "Cuadro generalizado de cubicaje", "cubicaje", id_cols=1))

    if path_masas:
        path_masas_latex = path_masas.replace('\\', '/')
        tex.append(r"\section{Diagrama de Masas (Curva Masa)}")
        tex.append(r"Evolución gráfica del volumen acumulado en función de la abscisa (Distancia en K).")
        tex.append(r"\begin{figure}[H]")
        tex.append(r"  \centering")
        tex.append(f"  \\includegraphics[width=0.95\\textwidth]{{{path_masas_latex}}}")
        tex.append(r"  \caption{Diagrama de Masas para compensación longitudinal de material}")
        tex.append(r"\end{figure}")

    if analisis_acarreo:
        tex.append(r"\section{Análisis de Acarreo}")
        dist_libre = analisis_acarreo.get('distancia_libre_km')
        costo_m3 = analisis_acarreo.get('costo_m3')
        if dist_libre is not None:
            tex.append(f"Se asume una distancia libre de acarreo de \\textbf{{{dist_libre:.2f} km}} sin costo "
                       "adicional de transporte; los volúmenes movilizados más allá de esta distancia deben "
                       "presupuestarse como transporte adicional según las especificaciones del proyecto.")
        if costo_m3 is not None:
            tex.append(f"El costo unitario de referencia suministrado es de \\textbf{{{costo_m3:.2f} por m$^3$}} "
                       "transportado más allá de la distancia libre. Esta cifra debe validarse contra el "
                       "presupuesto oficial vigente del proyecto.")

    tex.append(r"\section{Conclusiones y Dictamen Técnico}")
    dictamen, recomendaciones = evaluar_volumen(neto, corte, relleno, analisis_acarreo)
    texto_cierre = (r"El \textbf{Diagrama de Masas} demuestra los puntos críticos de corte y la distribución "
                     r"longitudinal del material, permitiendo a los ingenieros y planificadores viales organizar "
                     r"el acarreo en volquetas de manera eficiente.")
    tex.append(generar_seccion_recomendaciones(dictamen, recomendaciones, texto_cierre))

    tex.append(generar_referencias_normativas('volumenes'))

    # SECCIONES TRANSVERSALES MULTIPLES EN GRID (BOTTOM-UP)
    if paths_secciones and len(paths_secciones) > 0:
        tex.append(r"\newpage")
        tex.append(r"\section{Anexo Gráfico: Perfiles de Secciones Transversales}")
        tex.append(r"A continuación, se adjuntan los perfiles de todas las abscisas calculadas. Conforme a las normativas de presentación de planos de diseño vial, las secciones se han ploteado ordenadas de forma ascendente: \textbf{de abajo hacia arriba y de izquierda a derecha}.")

        paths_secciones = sorted(paths_secciones, key=lambda x: x[0])
        chunks = [paths_secciones[i:i+8] for i in range(0, len(paths_secciones), 8)]

        for idx_chunk, chunk in enumerate(chunks):
            tex.append(r"\begin{figure}[H]")
            tex.append(r"  \centering")

            # Formato estándar Bottom-Up, Left-to-Right en grilla 4x2
            grid_indices = [6, 7, 4, 5, 2, 3, 0, 1]

            for col_i, data_idx in enumerate(grid_indices):
                if data_idx < len(chunk):
                    abs_val, p_sec = chunk[data_idx]
                    p_sec_latex = p_sec.replace('\\', '/')
                    tex.append(r"  \begin{minipage}{0.48\textwidth}")
                    tex.append(f"    \\includegraphics[width=\\linewidth]{{{p_sec_latex}}}")
                    tex.append(r"  \end{minipage}")
                else:
                    tex.append(r"  \begin{minipage}{0.48\textwidth}")
                    tex.append(r"    \vspace{4.5cm}")
                    tex.append(r"  \end{minipage}")

                if col_i % 2 == 1:
                    tex.append(r"  \\[0.3cm]")
                else:
                    tex.append(r"  \hfill")

            tex.append(f"  \\caption{{Anexo Secciones Transversales - Plancha {idx_chunk+1}}}")
            tex.append(r"\end{figure}")
            if idx_chunk < len(chunks) - 1:
                tex.append(r"\newpage")

    tex.append(r"\end{document}")
    return "\n".join(tex)


# -------------------------------------------------------------------
# INFORME: NIVELACIÓN
# -------------------------------------------------------------------

def generar_reporte_nivelacion_latex(df_calc, metricas, tipo_nivelacion, autores, tutor,
                                      path_grafico=None, fotos_paths=None,
                                      metadata_campo=None, df_bancos=None, distancia_km=None,
                                      coef_precision_mm=10.0):
    """
    Parámetros nuevos (opcionales):
      metadata_campo: ver generar_ficha_tecnica_campo
      df_bancos: DataFrame opcional con los Bancos de Nivel (BM) utilizados (id, ubicación, cota oficial)
      distancia_km: longitud total del circuito, para evaluar tolerancia T = ek·√K
      coef_precision_mm: coeficiente ek (mm/√km) según el tipo de nivelación exigido
    """
    titulo = f"Informe Técnico de Altimetría\\\\ ({tipo_nivelacion})"
    tex = [generar_preambulo_y_caratula(titulo, autores, tutor)]

    error_cierre_mm = metricas.get('error_cierre_mm', 0)

    # --- RESUMEN EJECUTIVO ---
    pares_resumen = [
        ("Tipo de nivelación", escapar_latex(tipo_nivelacion)),
        ("Cota final calculada (cruda)", f"{metricas.get('cota_final_cruda', 0):.3f} m"),
        ("Cota teórica esperada", f"{metricas.get('cota_teorica_final', 0):.3f} m"),
        ("Error de cierre", f"{metricas.get('error_cierre_m', 0):.4f} m ({error_cierre_mm:.1f} mm)"),
    ]
    if distancia_km:
        pares_resumen.append(("Longitud del circuito", f"{distancia_km:.3f} km"))
    tex.append(generar_tabla_resumen_ejecutivo(pares_resumen))

    ficha = generar_ficha_tecnica_campo(metadata_campo)
    if ficha:
        tex.append(ficha)

    tex.append(r"\section{Marco Teórico y Referencia Altimétrica}")
    tex.append(r"El presente informe detalla el cálculo, ajuste y compensación de una red de apoyo altimétrico. El proceso se fundamenta en la Nivelación Geométrica o Directa, garantizando la transferencia de cotas desde un Banco de Nivel (BM) de origen hacia los puntos de interés.")

    if "Cerrada" in tipo_nivelacion:
        tex.append(r"Al tratarse de una Nivelación Cerrada, el circuito inicia y termina en el mismo punto de control, lo que permite cuantificar el error de cierre evaluando la diferencia entre la cota final calculada y la cota de partida.")
    else:
        tex.append(r"Al tratarse de una Nivelación Abierta con Control, la línea inicia en un Banco de Nivel conocido y cierra sobre un Banco de Nivel distinto, permitiendo contrastar la cota calculada de llegada con la elevación teórica esperada.")

    if df_bancos is not None and len(df_bancos) > 0:
        tex.append(r"\subsection{Bancos de Nivel (BM) Utilizados}")
        tex.append(r"Los siguientes puntos de control vertical fueron empleados como origen y/o cierre del circuito:")
        tex.append(dividir_y_generar_tablas(df_bancos, "Bancos de Nivel de Referencia", "bancos", id_cols=1))

    tex.append(r"\section{Cartera Altimétrica Compensada}")
    tex.append(r"A continuación se relacionan las lecturas de campo (vistas atrás, intermedias y adelante) junto con las cotas instrumentales y las elevaciones ajustadas tras el prorrateo del error de cierre:")

    df_clean = df_calc.drop(columns=['📸 Tomar_Fotos'], errors='ignore')
    tex.append(dividir_y_generar_tablas(df_clean, "Cartera de Nivelación Procesada", "nivelacion"))

    if fotos_paths and len(fotos_paths) > 0:
        tex.append(r"\subsection{Registro Fotográfico de Puntos Verticales}")
        tex.append(r"\begin{figure}[H]")
        tex.append(r"  \centering")
        for idx, path in enumerate(fotos_paths[:4]):
            path_latex = path.replace('\\', '/')
            tex.append(f"  \\includegraphics[height=5cm, keepaspectratio]{{{path_latex}}}")
            if idx % 2 == 1:
                tex.append(r"  \\[0.5cm]")
        tex.append(r"  \caption{Mosaico de registro fotográfico de placas/BMs}")
        tex.append(r"\end{figure}")

    tex.append(r"\section{Análisis de Errores y Compensación Altimétrica}")
    tex.append(r"Las métricas de validación geométrica del circuito arrojaron los siguientes resultados:")
    tex.append(r"\begin{itemize}")
    tex.append(f"  \\item \\textbf{{Sumatoria Vista Atrás ($\\Sigma V^+$):}} {metricas.get('sum_vista_atras', 0):.3f} m")
    tex.append(f"  \\item \\textbf{{Sumatoria Vista Adelante ($\\Sigma V^-$):}} {metricas.get('sum_vista_adelante', 0):.3f} m")
    tex.append(f"  \\item \\textbf{{Cota Final Calculada (Cruda):}} {metricas.get('cota_final_cruda', 0):.3f} m")
    tex.append(f"  \\item \\textbf{{Cota Teórica Esperada:}} {metricas.get('cota_teorica_final', 0):.3f} m")
    tex.append(f"  \\item \\textbf{{Error de Cierre Altimétrico:}} {metricas.get('error_cierre_m', 0):.4f} m ({error_cierre_mm:.1f} mm)")
    tex.append(r"\end{itemize}")

    if path_grafico:
        path_grafico_latex = path_grafico.replace('\\', '/')
        tex.append(r"\section{Perfil Topográfico de Nivelación}")
        tex.append(r"\begin{figure}[H]")
        tex.append(r"  \centering")
        tex.append(f"  \\includegraphics[width=0.95\\textwidth]{{{path_grafico_latex}}}")
        tex.append(r"  \caption{Perfil altimétrico de la línea de nivelación compensada}")
        tex.append(r"\end{figure}")

    tex.append(r"\section{Conclusiones y Dictamen Técnico}")
    dictamen, recomendaciones = evaluar_nivelacion(error_cierre_mm, distancia_km, coef_precision_mm)
    texto_cierre = (r"El ajuste altimétrico se ha distribuido de manera proporcional en los puntos de cambio, "
                     r"obteniendo un conjunto de elevaciones definitivas aptas para la densificación de cotas, "
                     r"control vertical y ejecución de obras civiles.")
    tex.append(generar_seccion_recomendaciones(dictamen, recomendaciones, texto_cierre))

    tex.append(generar_referencias_normativas('nivelacion'))

    tex.append(r"\end{document}")
    return "\n".join(tex)


# -------------------------------------------------------------------
# COMPILACIÓN
# -------------------------------------------------------------------

def compilar_latex_a_pdf(tex_code, output_dir="Reportes_PDF", filename="Reporte_Final"):
    os.makedirs(output_dir, exist_ok=True)

    safe_filename = re.sub(r'[^a-zA-Z0-9_\-]', '_', str(filename))
    safe_filename = re.sub(r'_+', '_', safe_filename).strip('_')
    if not safe_filename:
        safe_filename = "Reporte_Topografico"

    tex_path = os.path.join(output_dir, f"{safe_filename}.tex")
    pdf_path = os.path.join(output_dir, f"{safe_filename}.pdf")

    with open(tex_path, "w", encoding="utf-8") as f:
        f.write(tex_code)

    try:
        if shutil.which("pdflatex") is None:
            return None, tex_path, f"Error: no se encontró 'pdflatex' en el sistema ({platform.system()})."

        tex_path_seguro = tex_path.replace('\\', '/')
        output_dir_seguro = output_dir.replace('\\', '/')

        comando = [
            "pdflatex",
            "-interaction=nonstopmode",
            "-halt-on-error",
            f"-output-directory={output_dir_seguro}",
            tex_path_seguro
        ]

        # Primera pasada: genera el documento y valida errores de compilación.
        proceso = subprocess.run(comando, capture_output=True, text=True)

        if proceso.returncode != 0:
            log_error = proceso.stdout[-1500:] if proceso.stdout else proceso.stderr
            return None, tex_path, f"LaTeX falló al compilar el documento. Revisa el siguiente log de error:\n\n{log_error}"

        # Segunda pasada: resuelve referencias cruzadas (tabla de contenido, etc.)
        proceso_2 = subprocess.run(comando, capture_output=True, text=True)
        if proceso_2.returncode != 0:
            log_error = proceso_2.stdout[-1500:] if proceso_2.stdout else proceso_2.stderr
            return None, tex_path, ("La primera pasada de LaTeX fue exitosa, pero la segunda pasada "
                                     f"(resolución de referencias/índice) falló:\n\n{log_error}")

        if os.path.exists(pdf_path):
            with open(pdf_path, "rb") as f:
                pdf_bytes = f.read()
            return pdf_bytes, pdf_path, "OK"

        return None, tex_path, "LaTeX se ejecutó, pero no devolvió ningún archivo PDF."

    except Exception as e:
        return None, tex_path, f"Error en Python al llamar a LaTeX: {str(e)}"
