...
_PAQUETES_APT = {
    ...
    "pdflscape": "texlive-latex-recommended",
...
def tabla_larga(df, caption, label, formatos=None, notas=None,
                max_col_portrait=8, forzar_landscape=False, caps=None):
    """
    Tabla que se parte entre páginas repitiendo el encabezado.
    Con siunitx alinea por la coma decimal; sin siunitx alinea a la
    derecha y formatea los números en Python.
    """
    caps = caps or capacidades_latex()
    usa_siunitx = caps.get("siunitx", False)
    usa_lscape = caps.get("pdflscape", False)
    ...
    especificacion = []
    for c in cols:
        if es_numerica[c] and usa_siunitx:
    ...
    landscape = (forzar_landscape or len(cols) > max_col_portrait) and usa_lscape
    ...
    for c in cols:
        ...
        # En columnas S el encabezado debe ir entre llaves
        heads.append("{" + txt + "}" if usa_siunitx else txt)
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        for c in cols:
            val = row[c]
            if pd.isna(val):
                celdas.append("{---}" if (es_numerica[c] and usa_siunitx) else "---")
            elif es_numerica[c]:
                dec, _ = decidir_formato(c, formatos)
                crudo = numero_plano(val, dec)
                celdas.append(crudo if usa_siunitx else _num_es(crudo))
            else:
...
def preambulo_v2(ruta_logo="Iconos/logo_geopol.png",
    ...
    for paq in ("booktabs", "float", "caption", "threeparttable",
                "pdflscape", "enumitem", "csquotes", "titlesec",
    ...
    if hay("siunitx"):
        tex.append(r"\sisetup{output-decimal-marker={,}, group-separator={.},"
                   r" group-minimum-digits=4, detect-weight=true,"
                   r" detect-family=true, table-align-text-before=false}")
